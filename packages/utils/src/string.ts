...
const ENTITY_REGEX = /&[a-zA-Z0-9#x]+;/gi;
const NUMERIC_ENTITY_REGEX = /^&#([0-9]+|[xX][a-fA-F0-9]+);$/;
...
export function decodeHtmlEntities(text: string): string {
  ...
  return text.replace(ENTITY_REGEX, (match) => {
    ...
    if (numericMatch) {
      ...
      const code = isHex ? numericMatch[1].slice(1) : numericMatch[1];
      // Use fromCodePoint for Emoji/Astral support
      return String.fromCodePoint(Number.parseInt(code, base));
    }
