...
export function decodeHtmlEntities(text: string): string {
  ...
  return text.replace(ENTITY_REGEX, (match) => {
    ...
    if (NUMERIC_ENTITY_REGEX.test(match)) {
      const codePart = match.slice(2, -1);
      const isHex = codePart.toLowerCase().startsWith('x');
      const code = isHex ? Number.parseInt(codePart.slice(1), 16) : Number.parseInt(codePart, 10);
