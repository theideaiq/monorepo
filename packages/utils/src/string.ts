...
export function decodeHtmlEntities(text: string): string {
  ...
  return text.replace(ENTITY_REGEX, (match) => {
    ...
    if (numericMatch) {
      const codePointStr = numericMatch[1];
      const isHex = codePointStr.toLowerCase().startsWith('x');
      const base = isHex ? 16 : 10;
      const parsed = isHex ? codePointStr.substring(1) : codePointStr;

      // Use fromCodePoint for Emoji/Astral support
      return String.fromCodePoint(Number.parseInt(parsed, base));
    }
