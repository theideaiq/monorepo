...
## 2024-05-24 - Sitemap Synchronization
**Learning:** Found a discrepancy between `sitemap.ts` hardcoded routes and actual application routes (e.g., `/club` missing, `/suite` giving 404s). This causes crawling errors (404s) and poor indexability of core pages.
**Action:** When adding or removing top-level pages, always update `apps/web/src/app/sitemap.ts` to ensure it accurately reflects the current site structure to avoid crawler errors.
