...
export function ProductCard({
  ...
  return (
    <motion.div
      whileHover={{ y: -5 }}
      className="group relative h-full bg-white/5 border border-white/5 rounded-2xl overflow-hidden hover:border-brand-yellow/30 hover:shadow-[0_10px_30px_-10px_rgba(0,0,0,0.5)] transition-all duration-300 flex flex-col"
    >
      <Link
        href={`/product/${product.slug}`}
        className="absolute inset-0 z-10 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-yellow rounded-2xl"
      ...
      <div className="relative aspect-square bg-[#1a1a1a] overflow-hidden">
        ...
        <button
          ...
          className="absolute bottom-3 right-3 p-3 bg-brand-yellow text-brand-dark rounded-full shadow-lg translate-y-12 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:translate-y-0 transition-all duration-300 hover:bg-white hover:text-black z-20"
