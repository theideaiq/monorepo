...
describe('Cart Store', () => {
  ...
  it('should add items to the cart', () => {
    ...
    const apple = { id: 'apple', productId: 'p1', title: 'Apple', price: 1, image: 'apple.png' };
    addItem(apple);
    expect(useCartStore.getState().items).toEqual([{ ...apple, quantity: 1 }]);

    const banana = { id: 'banana', productId: 'p2', title: 'Banana', price: 2, image: 'banana.png' };
    addItem(banana);
  ...
  it('should persist state to localStorage', () => {
    const { addItem } = useCartStore.getState();
    const item = { id: 'persistent-item', productId: 'p1', title: 'Item', price: 1, image: 'item.png' };
    addItem(item);
