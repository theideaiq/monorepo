...
export function NewJournalEntryModal({
  ...
  const handleLineChange = (index: number, field: keyof typeof lines[0], value: any) => {
    ...
    const prevLine = newLines[index] || { accountId: '', debit: 0, credit: 0 };
    newLines[index] = { ...prevLine, [field]: value };
    setLines(newLines);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createJournalEntry(date as string, description as string, validLines as { accountId: string; debit: number; credit: number }[]);
