...
export function NewJournalEntryModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Cast the mapped array strictly to appease TS since we checked .accountId above
      const strictLines = lines.map((l) => ({
        accountId: l.accountId as string,
        debit: Number(l.debit),
        credit: Number(l.credit),
      }));

      await createJournalEntry(date, description, strictLines);
