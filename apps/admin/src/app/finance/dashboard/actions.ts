...
export async function getCashFlowData(year: number) {
  ...
  lines.forEach((line: any) => {
    ...
    if (!month) return; // Should not happen given getMonth() returns 0-11, but protects against undefined
    ...
    // We can confidently assert monthlyData[month] is defined now
    if (type === 'revenue') {
